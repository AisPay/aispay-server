...
export default async function callback(fastify: FastifyInstance, opts: FastifyPluginOptions, next: (err?: Error | undefined) => void) {
  ...
  fastify.withTypeProvider<ZodTypeProvider>().get("/refresh", {schema: {response: refreshResponse}}, userController.refresh);
  fastify.withTypeProvider<ZodTypeProvider>().get("/logout", {schema: {response: logoutResponse}}, userController.logout);
  next();
}
